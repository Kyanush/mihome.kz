...
$html = file_get_html('https://mi-home.kz/store/stil-zhizni/xiaomi-90-points-vitality-casual-backpack');


$name 			  = $html->find('h1#product-name', 0)->plaintext;
$sku 			  = $html->find('span[itemprop=sku]', 0)->plaintext;
$description_mini = $html->find('div[itemprop=description]', 0)->plaintext;
$price 			  = $html->find('div.PricesalesPrice', 0)->plaintext;
$status 		  = $html->find('div.uk-alert-success', 0)->plaintext;
$description 	  = $html->find('div.product-description', 0)->innertext;


$description_object = $html->find('div.product-description', 0);
foreach($description_object->find('img') as $element)
{
	echo $element->{'data-src'} . '<br/>';
}


$images = [];

$thumbnails = $html->find('ul#thumbnails', 0);
foreach($thumbnails->find('img') as $element)
{
	$images[] = $element->{'data-src'};
}


$attributes = [];
$a1 = $html->find('ul#tab-content', 0);
$a1 = $a1->children(1);
